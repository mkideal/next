...
func (o *Options) SetupCommandFlags(flagSet *flag.FlagSet, u flags.UsageFunc) {
	...
	// The trace message is used for debugging the compiler itself.
	// Verbosity **1** (debug) and above enable execution of:
	//
	...
	flagSet.IntVar(&o.Verbose, "v", 0, u(""+
		"Control `VERBOSITY` of compiler output and debugging information: "+b("0")+"=error,  "+b("1")+"=debug, "+b("2")+"=trace.\n"+
		"The trace message used for debugging the compiler itself. Verbosity "+b("1")+" and above enable execution of:\n\n"+
	...
	// @api(CommandLine/Flag/-grammar) represents the custom grammar for the next source code.
	//
	// Example:
	//
	//	```sh
	...
	//	:::
	flagSet.StringVar(&o.Grammar, "grammar", "", u(""+
		...
		"by your requirements. The grammar file is a YAML or JSON file that contains rules.\n",
	))
	...
	// @api(CommandLine/Flag/-X) represents the custom annotation solver commands for code generation.
	// Annotation solvers are executed in a separate process to solve annotations.
	// All annotations are passed to the solver command via stdin and stdout.
	...
	// :::tip
	//
	// In the example above, the `message-type-allocator` is a custom annotation solver command that
	...
	flagSet.Var(&o.Solvers, "X", u(""+
		"Specify custom annotation solver commands for code generation.\n"+
		"`ANNOTATION=COMMAND` defines the target annotation and its solver command.\n"+
		"Annotation solvers are executed in a separate process to solve annotations.\n"+
		"All annotations are passed to the solver command via stdin and stdout.\n"+
