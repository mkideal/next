...
<Tabs
  ...
  :::note
  You alse can install Next using Git-Bash following **Unix-like**:

  ```sh
  curl -fsSL https://getnext.sh | sh
  ```

  Or, download a `.msi` installer from [Downloads](/docs/downloads) for windows.
  :::
  </TabItem>
</Tabs>
...
Currently, Next has created some builtin `map` files and base templates. See [builtin](https://github.com/mkideal/next/tree/main/builtin) for more informarion.
