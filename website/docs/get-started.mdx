...
```next title="example.next"
@next(
    go_package="github.com/next/next/gen/go/demo",
    cpp_package="demo",
    c_package="DEMO_",
...
```npl title="example.cpp.npl"
{{head}}

{{next this}}
```

- **`{{head}}`**: Generate a header like this: `// Code generated by "next v0.0.3"; DO NOT EDIT.`
- **`{{next this}}`**: Generate `this` declaration. By default, `this` is a file if the `meta/this` not specified.

:::tip
See [API/Context/meta](/docs/api/context#user-content-Context_meta) for details about `meta`.
:::
...
<Tabs
  ...
  This template file is used to generate all constants in a file `Constants.java` which define a `Constants` class to holds all constants.
  ```npl
...
public class Constants {
    ...
    public static final long MAX_RETRIES = 3L; // Integer constant
