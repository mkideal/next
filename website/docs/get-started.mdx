...
using {{render "package:namespace" .File.Package}};
{{- end}}

{{/* next/csharp/const: Generates a constant declaration */}}
{{- define "next/csharp/const" -}}
{{next .Doc}}public const {{next .Type}} {{render "const:name" .}} = {{next .Value}};{{next .Comment}}
{{- end}}
...
{{next .Doc}}{{render "enum.member:name" .}} = {{next .Value}},{{next .Comment}}
{{- end}}
...
{{next .Doc}}public {{next .Type}} {{render "struct.field:name" .}} { get; set; }{{next .Comment}}
{{- end}}
...
{{next .Doc}}{{next .Result}} {{render "interface.method:name" .}}({{next .Params}});{{next .Comment}}
{{- end}}
