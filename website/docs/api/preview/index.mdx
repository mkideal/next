...
export const version = () => {
  const versionString = useDocusaurusContext().siteConfig.customFields.version;
  const parts = versionString.split(".");
  if (parts.length === 2) {
    return `${versionString}.1`;
  } else if (parts.length === 3) {
    const lastNumber = parseInt(parts[2], 10);
    parts[2] = (lastNumber + 1).toString();
    return parts.join(".");
  } else {
...
## Introduction to Next {version()}

:::note
**Next {version()} is not yet released. These are work-in-progress release notes.**
:::
